...
static real_T c_emlrt_marshallIn(const emlrtStack *sp, const mxArray *sel, const
  char_T *identifier);
  ...
  emlrtMsgIdentifier *parentId))[64]
{
  ...
  static real_T c_emlrt_marshallIn(const emlrtStack *sp, const mxArray *sel,
  const char_T *identifier)
{
  ...
  y = d_emlrt_marshallIn(sp, emlrtAlias(sel), &thisId);
  emlrtDestroyArray(&sel);
  return y;
}
...
void ApproxMatMult8x8_api(const mxArray * const prhs[3], const mxArray *plhs[1])
{
  real_T (*res)[64];
  real_T (*T)[64];
  real_T (*I)[64];
  ...
  sel = c_emlrt_marshallIn(&st, emlrtAliasP(prhs[2]), "sel");

  /* Invoke the target function */
  ApproxMatMult8x8(&st, *T, *I, sel, *res);
