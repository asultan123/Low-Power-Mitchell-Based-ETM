function res = mult_ETM_int(a,b,selector)

    a = uint16(a);
    b = uint16(b);
    upper_a = uint8(uint16(bitand(a,65280))*2^-8);
    upper_b = uint8(uint16(bitand(b,65280))*2^-8);
...
%         res = bitor(bitshift(uint32(upper_b),16),res);
    ...
    upper_mult = uint32(mult_selector(upper_a,upper_b,selector))*2^16;
